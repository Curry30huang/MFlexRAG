...
class ImageRetrieverAgent():
    ...
    """
    def __init__(self, dataset: str, node_dir_prefix: str, embed_model_name: str, gmm: bool = False, input_gmm: int = 20, max_output_gmm: int = 10, min_output_gmm: int = 5, start_idx:int=1, end_idx:int=999999) -> None:
        """
        初始化检索代理
    ...
    def search(self, doc_name: str, query: str,start_idx:int=1, end_idx:int=999999) -> Optional[List[Dict[str, Any]]]:
        """
        根据query 检索指定的文档页面
        Args:
            doc_name: 文档名称
        ...
        if self.search_engine is None:
            self.search_engine = SearchEngine(self.dataset, doc_name, self.node_dir_prefix, self.embed_model_name, start_idx, end_idx)
            if self.gmm:
            ...
            self.search_engine.change_document(doc_name,start_idx,end_idx)
